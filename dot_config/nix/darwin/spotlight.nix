{ primaryUser, lib, ... }:
# Spotlight management — NOT in the base. Writing Spotlight exclusions edits the
# SIP-protected /System/Volumes/Data/.Spotlight-V100/VolumeConfiguration.plist
# and kickstarts mds, which require the activating terminal to have Full Disk
# Access. On a host without FDA the activation step errors ("Operation not
# permitted while SIP is engaged") and fails the whole switch. So a host opts in
# from its hosts/<host>/configuration.nix only when it actually wants this (and
# has granted FDA). See darwin/default.nix (demoted) and hosts/bearcat.
let
