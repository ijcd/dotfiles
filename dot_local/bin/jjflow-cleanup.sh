# jjflow-cleanup.sh — jj-cleanup logic as a SOURCED module. usage → cleanup_usage,
# main → cleanup_main (clears jj-flow's namespace). cleanup_main does `set +e`
# because the original ran without -e (jj queries return non-zero legitimately).
cleanup_usage() {
  cat <<'EOF'
...
# GitHub repo (owner/name) for gh queries. jj workspaces here aren't git-colocated,
# so gh can't infer the repo from cwd — always pass -R. Source order:
# $JJ_CLEANUP_REPO → jj-vine's configured project → origin URL.
gh_repo() {
  local r="${JJ_CLEANUP_REPO:-}"
  [[ -n "$r" ]] && { printf '%s\n' "$r"; return; }
  r="$(jj config get jj-vine.github.project 2>/dev/null)"
  [[ -n "$r" ]] && { printf '%s\n' "$r"; return; }
  jj git remote list --ignore-working-copy 2>/dev/null | awk '/^origin /{print $2}' \
    | sed -E 's#(git@github\.com:|https://github\.com/)##; s#\.git$##'
}

pr_state() { # <suffix> -> MERGED|CLOSED|OPEN|NONE|ERROR
  local suffix=$1 repo json state
  repo="$(gh_repo)"
  [[ -n "$repo" ]] || { printf 'ERROR\n'; return; }
  # pr list --head --state all finds MERGED/CLOSED PRs even after the head branch
  # was auto-deleted (pr view <branch> misses those). Returns a JSON array ([] = none).
  json="$(gh pr list -R "$repo" --head "ijcd/$suffix" --state all --json state,mergedAt --limit 1 2>/dev/null)" \
    || { printf 'ERROR\n'; return; }
  [[ -n "$json" && "$json" != "[]" ]] || { printf 'NONE\n'; return; }
...
# (Hermetic-test deviation, see plan Task 4: <base>.. not trunk()..)
prime_count() { # <suffix>
...
ws_dirty() { # <name>  -> exit 0 if that workspace @ has uncommitted diff
  local name=$1
  # jj diff -s (summary) is empty when clean; --stat always prints "0 files changed".
  [[ -n "$(jj diff --ignore-working-copy -r "${name}@" -s 2>/dev/null)" ]]
}
...
do_park() { # <suffix> <todo-name>
  ...
  # merged middle; then bookmark the rebased tip. After rebase, wip/<suffix> follows
  # its commits to the new location, so -r wip/<suffix> is the rebased tip.
...
rm_ws_dir() { # <root>
  ...
  # Canonicalize the prefix too so /tmp vs /private/tmp (macOS) compares equal.
  [[ -d "$prefix" ]] && prefix="$(cd "$prefix" && pwd -P)"
...
del_remote() { # <suffix>
  # jj git push --deleted takes no --bookmark; delete the local bookmark then push
  # that one bookmark to propagate the deletion. Tolerates already-gone.
  local s=$1
  jj bookmark delete "ijcd/$s" >/dev/null 2>&1 || true
  jj git push --bookmark "ijcd/$s" >/dev/null 2>&1 \
...
cleanup_main() {
  set +e   # original tool ran without -e; jj queries return non-zero legitimately
  ...
  case "$cmd" in
    ...
    __test_pr_state)        pr_state "$@";;
    __test_prime_count)     prime_count "$@";;
    __test_lost_tail_root)  lost_tail_root "$@";;
    __test_ws_dirty)        ws_dirty "$@";;
    __test_do_park)         do_park "$@";;
